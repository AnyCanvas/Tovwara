...
function totalJson($day,$month,$year,$fnbtId,$clientId){
	...
	}else if( $clientId !== 0 && $_SESSION['Id'] !== '00'){
		$sql = "SELECT * FROM interactions WHERE EXTRACT(MONTH FROM date) = '". $month. "' AND EXTRACT(YEAR FROM date) = '". $year."' AND clientId='".$_SESSION['Id'] ."'"; 
	} else{
		$sql = "SELECT * FROM interactions WHERE EXTRACT(MONTH FROM date) = '". $month. "' AND EXTRACT(YEAR FROM date) = '". $year."'"; 
	}
	$result = $conn->query($sql);
